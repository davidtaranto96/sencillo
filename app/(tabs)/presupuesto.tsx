...
export default function PresupuestoScreen() {
  ...
  const [salaryRule, setSalaryRule] = React.useState(budgetService.getSalaryRule());
  ...
  useEffect(() => {
    loadCategories();
    const rule = budgetService.getSalaryRule();
    setSalaryRule(rule);
    const spending = transactionService.getCategorySpending(now.getFullYear(), now.getMonth() + 1);
    setCategorySpending(spending);
    const income = budgetService.getMonthlyIncome(now.getFullYear(), now.getMonth() + 1);
